...
function buildRecordsBaseSql(filters = {}, authUser = null) {
  let sql = `
  ...
  if (authUser?.role === 'paciente') {
    sql += ` AND mr.paciente_id = ?`;
    params.push(Number(authUser.id));
  }

  if (filters.search && String(filters.search).trim()) {
    sql += ` AND (CONCAT_WS(' ', p.nombre, p.apellidos) LIKE ? OR p.matricula LIKE ? OR p.email LIKE ?)`;
    const value = `%${String(filters.search).trim()}%`;
    params.push(value, value, value);
  }

  if (filters.doctor_id) {
    sql += ` AND EXISTS (SELECT 1 FROM consultations c_filter WHERE c_filter.record_id = mr.id AND c_filter.doctor_id = ?)`;
    params.push(Number(filters.doctor_id));
  }

  if (filters.fecha_inicio) {
    sql += ` AND DATE(COALESCE(lastc.consultation_date, mr.created_at)) >= ?`;
...
async function findAll(filters = {}, authUser = null) {
  const { sql, params } = buildRecordsBaseSql(filters, authUser);
  const [rows] = await db.execute(
    `
      ...
      GROUP BY mr.id, p.id, lastc.id, ldu.id
...
async function findById(recordId, authUser = null) {
  ...
  if (authUser?.role === 'medico') {
    const [allowed] = await db.execute(
      `SELECT 1
       FROM appointments
       WHERE paciente_id = ? AND doctor_id = ? AND estado IN ('confirmada', 'atendida')
       LIMIT 1`,
...
async function findByPatientId(patientId, authUser = null) {
  const [rows] = await db.execute(
    `SELECT id FROM medical_records WHERE paciente_id = ? LIMIT 1`,
    [Number(patientId)]
  );
